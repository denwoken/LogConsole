...
void ConsoleFormatter::formatBlocksToDocs_thread(QList<QVector<LogLine>> *blocks)
{
    if(!blocks){
        emit sigBlocksProcessingEnd();
        return;
    }
    for(int i = blocks->size()-1; i >= 0; i--){
        QTextDocument* d = formatBlockToDoc((*blocks)[i]);
        //qDebug() << "ready new block";
        emit sigBlockReady(d);
    }
    emit sigBlocksProcessingEnd();
}
...
LogConsoleWidget::LogConsoleWidget(QWidget *parent) : QWidget(parent), ui(new Ui::LogConsoleWidget)
{
    ...
    qRegisterMetaType<QList<QVector<LogLine>>*>("QList<QVector<LogLine>>*");
...
bool LogConsoleWidget::saveSettings(QString path) const
{
...
void LogConsoleWidget::insertFrontDocumentFragment(QTextDocumentFragment fragment)
{
    QMutexLocker locker(&m_mutex);
    QTextCursor cursor = ui->textEdit->textCursor();
...
void LogConsoleWidget::insertFrontDocument(QTextDocument *doc)
{
    QMutexLocker locker(&m_mutex);
    QTextCursor cursor = ui->textEdit->textCursor();
...
*/
    ...
    ConsoleFormatter* formatter = new ConsoleFormatter(&m_settings);
    QThread* thread = new QThread();
    formatter->moveToThread(thread);
    thread->start();


    connect(formatter, &ConsoleFormatter::sigBlockReady,
           this, &LogConsoleWidget::insertFrontDocument,
           Qt::QueuedConnection);

    QMetaObject::invokeMethod(formatter, "formatBlocksToDocs_thread",
                              Qt::ConnectionType::AutoConnection,
                              Q_ARG(QList<QVector<LogLine>>*, &blocks));
    QEventLoop loop;
    QObject::connect(formatter, &ConsoleFormatter::sigBlocksProcessingEnd,
                     &loop, &QEventLoop::quit);
    qDebug() << "start loop";
    loop.exec();// Запускаем цикл обработки событий

    std::cout << "loop end " << timer.nsecsElapsed()/1000/1000 << " ms;" << std::endl;

    formatter->deleteLater();
    thread->terminate();
    thread->deleteLater();

    std::cout << "all time " << timer.nsecsElapsed()/1000/1000 << " ms;" << std::endl;
}
